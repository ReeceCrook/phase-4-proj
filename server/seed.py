...
with app.app_context():
        ...
        for _ in range(random.randint(1, 2)):
            ...
            for shared_user in shared_users:
                if not primary_owner_chosen:
                    shared_blog_instance = shared_blog.insert().values(
                        user_id=shared_user.id,
                        blog_id=new_blog.id,
                        primary_owner=user.username,
                        co_owner=shared_user.username
                    )
                    ...
                    shared_blog_instance = shared_blog.insert().values(
                        user_id=shared_user.id,
                        blog_id=new_blog.id,
                        primary_owner=shared_user.username,
                        co_owner=user.username
                    )
                    db.session.execute(shared_blog_instance)
    ...
    for blog in new_blogs:
        for _ in range(random.randint(1, 2)):
            ...
            user = random.choice(blog.owner.shared_blogs) if blog.users else blog.owner
            new_post = Post(title=title, description=description, content=content, user=user, blog=blog)
